...
class WarframeRepository {
    ...
    suspend fun searchAuctions(weaponUrlName: String): Result<List<Auction>> = withContext(Dispatchers.IO) {
        try {
            val response = marketApi.searchAuctions(weaponUrlName = weaponUrlName)
            Result.success(response.payload.auctions)
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
